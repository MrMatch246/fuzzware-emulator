...
UNICORN_QEMU_FLAGS="--python=/usr/bin/pypy3" make -C afl clean all || exit 1
...
pypy3 -m pip install -U cython || exit 1
pypy3 -m pip install -U -r requirements.txt || exit 1

pushd harness; pypy3 -m pip install -e . || { popd; exit 1; }; popd
exit 0